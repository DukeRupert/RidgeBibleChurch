<script>
  ...
  const stroke = "stroke:#b61200; stroke-width: 1";
  const fill = 'stroke:#b60000; fill: #b60000';
...
{#if condition}
  ...
  >
    <g transform="translate(-16.4 -40.2)" fill="#b60000" stroke-width=".0847">
      ...
      <g fill='#FFFFFF'>
        <path in:draw="{{delay: 300, duration: 3000, easing: quartIn}}" style={stroke} d={ridgeline}>
            <title>ridgeline</title>
        </path>
        <path in:fade="{{delay: 2400, duration: 900, easing: sineIn}}" style={fill} d={ridgeline}>
            <title>ridgeline</title>
        </path>
      </g>
      <path d={bibleChurch}>
